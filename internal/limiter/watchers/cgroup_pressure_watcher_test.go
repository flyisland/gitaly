...
func TestPSIResourceConfig_Defaults(t *testing.T) {
	...
	require.Equal(t, 5.0, mem.WarningThreshold)
	require.Equal(t, 10.0, mem.BackoffThreshold)
	require.Equal(t, 25.0, mem.CriticalThreshold)
	require.Equal(t, 60, mem.SustainDurationSeconds)
	...
	require.Equal(t, 10.0, cpu.WarningThreshold)
...
func TestCgroupPressureWatcher_Poll(t *testing.T) {
	...
	t.Run("severity classification", func(t *testing.T) {
		...
		for _, tc := range []struct {
			...
			{name: "warning", avg60: 7.0, wantLogs: 1, wantMsg: "PSI pressure above warning threshold", wantSev: PSISeverityWarning},
			{name: "backoff", avg60: 12.0, wantLogs: 1, wantMsg: "PSI pressure at backoff threshold", wantSev: PSISeverityBackoff},
			{name: "critical", avg60: 28.0, wantLogs: 1, wantMsg: "Critical PSI pressure detected", wantSev: PSISeverityCritical},
		} {
	...
	t.Run("backoff-level pressure logs with 3-condition fields and all PSI metrics", func(t *testing.T) {
		...
		w := NewCgroupPressureWatcher(
			&testCgroupManager{
				...
				statsList: []cgroups.Stats{{
					ParentStats: cgroups.CgroupStats{
						MemoryPSI: cgroups.PSIMetrics{
							Some: cgroups.PSIData{Avg10: 15.0, Avg60: 12.0, Avg300: 4.5},
		...
		require.Equal(t, 15.0, data["memory_pressure_some_avg10"])
		require.Equal(t, 12.0, data["memory_pressure_some_avg60"])
		require.Equal(t, 4.5, data["memory_pressure_some_avg300"])
		...
		require.Equal(t, 10.0, data["psi_threshold_backoff"])
		require.Equal(t, 25.0, data["psi_threshold_critical"])
	})
	...
	t.Run("never sends backoff event", func(t *testing.T) {
		...
		mgr := &testCgroupManager{
			...
			statsList: []cgroups.Stats{
				{ParentStats: cgroups.CgroupStats{
					MemoryPSI: cgroups.PSIMetrics{
						Some: cgroups.PSIData{Avg10: 30.0, Avg60: 28.0},
					},
				}},
				{ParentStats: cgroups.CgroupStats{
					MemoryPSI: cgroups.PSIMetrics{
						Some: cgroups.PSIData{Avg10: 32.0, Avg60: 30.0},
					},
				}},
			},
		}
	...
	t.Run("CPU resource logs correctly", func(t *testing.T) {
		...
		w := NewCgroupPressureWatcher(
			&testCgroupManager{
				...
				statsList: []cgroups.Stats{{
					ParentStats: cgroups.CgroupStats{
						CPUPSI: cgroups.PSIMetrics{
							Some: cgroups.PSIData{Avg10: 20.0, Avg60: 16.0, Avg300: 9.3},
						},
					},
				}},
			},
		...
		require.Equal(t, 20.0, hook.LastEntry().Data["cpu_pressure_some_avg10"])
		require.Equal(t, 16.0, hook.LastEntry().Data["cpu_pressure_some_avg60"])
		require.Equal(t, 9.3, hook.LastEntry().Data["cpu_pressure_some_avg300"])
	})
	...
	t.Run("Memory resource logs correctly", func(t *testing.T) {
		...
		require.Equal(t, 10.0, hook.LastEntry().Data["psi_threshold_backoff"])
		require.Equal(t, 25.0, hook.LastEntry().Data["psi_threshold_critical"])
	})
