...
func TestCgroupMemoryWatcher_Name(t *testing.T) {
	...
	manager := NewCgroupMemoryWatcher(&testCgroupManager{}, 0.6)
...
func TestCgroupMemoryWatcher_Poll(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			desc: "cgroup anon memory usage is more than or equal to 60%",
			manager: &testCgroupManager{
				ready: true,
				statsList: []cgroups.Stats{
					{
			...
			expectedEvent: &limiter.BackoffEvent{
				...
				Stats: map[string]any{
					...
					"memory_pressure_some_avg300": float64(0),
					"memory_pressure_full_avg10":  float64(0),
					"memory_pressure_full_avg60":  float64(0),
					"memory_pressure_full_avg300": float64(0),
					"io_pressure_some_avg10":      float64(0),
					"io_pressure_some_avg60":      float64(0),
					"io_pressure_some_avg300":     float64(0),
					"io_pressure_full_avg10":      float64(0),
					"io_pressure_full_avg60":      float64(0),
					"io_pressure_full_avg300":     float64(0),
					"cpu_pressure_some_avg10":     float64(0),
					"cpu_pressure_some_avg60":     float64(0),
					"cpu_pressure_some_avg300":    float64(0),
					"pgmajfault":                  uint64(0),
				},
			},
			expectedErr: nil,
		...
		{
			desc: "high memory usage but low anon does not trigger backoff",
			manager: &testCgroupManager{
				ready: true,
				statsList: []cgroups.Stats{
					{
		...
		{
			desc: "cgroup anon memory usage normal",
			manager: &testCgroupManager{
				ready: true,
				statsList: []cgroups.Stats{
					{
						ParentStats: cgroups.CgroupStats{
							TotalAnon:   1100000000,
							MemoryLimit: 2000000000,
						},
					},
				},
