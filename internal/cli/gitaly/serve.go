...
import (
	...
	"gitlab.com/gitlab-org/gitaly/v18/internal/gitaly/maintenance"
	...
	"gitlab.com/gitlab-org/gitaly/v18/internal/gitaly/storage/storagemgr/partition/migration/reftable"
	...
	housekeepingmw "gitlab.com/gitlab-org/gitaly/v18/internal/grpc/middleware/housekeeping"
	...
	_ "gitlab.com/gitlab-org/gitaly/v18/internal/grpc/proxy"
)
...
func configure(configPath string) (config.Cfg, log.Logger, error) {
	...
	errorTruncator := &log.ErrorTruncatorHook{MaxBytes: 1024}

	logger, err := log.Configure(
...
func run(appCtx *cli.Command, cfg config.Cfg, logger log.Logger) error {
	...
	if cfg.Transactions.Enabled {
		...
		dryRunLogCache, err := storagemgr.NewDryRunLogCache(60*time.Minute, 15000)
		if err != nil {
