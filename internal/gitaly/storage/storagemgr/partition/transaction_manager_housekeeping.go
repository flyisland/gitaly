...
func (mgr *TransactionManager) prepareRepacking(ctx context.Context, transaction *Transaction) error {
	...
	workingRepository := mgr.repositoryFactory.Build(transaction.relativePath)
	repoPath := mgr.getAbsolutePath(ctx, workingRepository.GetRelativePath())
...
func (mgr *TransactionManager) prepareCommitGraphs(ctx context.Context, transaction *Transaction) error {
	...
	if err := housekeeping.WriteCommitGraph(ctx,
		mgr.repositoryFactory.Build(transaction.relativePath),
...
func (mgr *TransactionManager) verifyPackRefs(ctx context.Context, transaction *Transaction, refBackend git.ReferenceBackend, zeroOID git.ObjectID) (*gitalypb.LogEntry_Housekeeping_PackRefs, error) {
	...
	if refBackend == git.ReferenceBackendReftables {
		packRefs, err := mgr.verifyPackRefsReftable(ctx, transaction)
		if err != nil {
...
func (mgr *TransactionManager) verifyRepacking(ctx context.Context, transaction *Transaction) (returnedErr error) {
	...
	stagingCtx, stagingRepository, err := mgr.setupStagingRepository(ctx, transaction)
	if err != nil {
	...
	if err := mgr.verifyObjectsExist(stagingCtx, stagingRepository, objectDependencies); err != nil {
...
func (mgr *TransactionManager) verifyPackRefsReftable(ctx context.Context, transaction *Transaction) (*gitalypb.LogEntry_Housekeeping_PackRefs, error) {
	...
	repoPath := mgr.getAbsolutePath(ctx, transaction.relativePath, storage.WithRootStorage())
	newTableList, err := reftable.ReadTablesList(repoPath)
	if err != nil {
		return nil, fmt.Errorf("reading tables.list: %w", err)
	}

	snapshotRepoPath := mgr.getAbsolutePath(storage.ContextWithTransaction(context.Background(), transaction), transaction.snapshotRepository.GetRelativePath())
...
func (mgr *TransactionManager) verifyPackRefsFiles(ctx context.Context, transaction *Transaction, zeroOID git.ObjectID) (*gitalypb.LogEntry_Housekeeping_PackRefs, error) {
	...
	if err := prunedPaths.WalkPostOrder(func(path string, isDir bool) error {
		...
		if isDir {
			...
			entries, err := os.ReadDir(mgr.getAbsolutePath(ctx, relativePath, storage.WithRootStorage()))
			if err != nil {
